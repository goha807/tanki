...
// Додамо колонку coins та upgrades, якщо їх немає (бажано виконати в SQL: ALTER TABLE users ADD COLUMN coins INT DEFAULT 0)
db.connect();
...
setInterval(() => {
    bullets.forEach((b, index) => {
        b.x += b.dx; b.y += b.dy;
        b.dist -= Math.sqrt(b.dx**2 + b.dy**2); // Зменшення дальності
        ...
        for (let id in players) {
            ...
            if (b.x > p.x && b.x < p.x + 30 && b.y > p.y && b.y < p.y + 30) {
                ...
                bullets.splice(index, 1);
                if (p.hp <= 0) {
                    p.alive = false; // Гравець "мертвий" до респавну
                    if (players[b.owner]) {
                        players[b.owner].score += 10;
                        players[b.owner].coins += 50; // +50 монет за вбивство
                        db.query('UPDATE users SET score = score + 10, coins = coins + 50 WHERE username = ?', [players[b.owner].username]);
                    }
                    io.to(id).emit('died'); // Надсилаємо сигнал смерті
                }
...
io.on('connection', (socket) => {
    socket.on('joinGame', (user) => {
        players[socket.id] = { 
            ...
            speed: 4, fireRate: 500, range: 600, alive: true, photo: user.photo || null
        };
        io.emit('updatePlayers', players);
    });
    ...
    socket.on('upgrade', (type) => {
        ...
        if (!p || p.coins < 100) return;
        if (type === 'speed') { p.speed += 0.5; p.coins -= 100; }
        if (type === 'fireRate') { p.fireRate = Math.max(150, p.fireRate - 50); p.coins -= 100; }
        if (type === 'range') { p.range += 100; p.coins -= 100; }
        socket.emit('updateBalance', p.coins);
        io.emit('updatePlayers', players);
    });
    ...
    socket.on('shoot', (data) => {
        const p = players[socket.id];
        if (p && p.alive) {
            bullets.push({ ...data, owner: socket.id, dist: p.range });
        }
    });

    socket.on('updatePhoto', (url) => {
